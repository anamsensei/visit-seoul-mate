...
function schedule(places,{startHour=11,duration=8}={}){
  ...
  function walk(left,route,now,length,mealIndex){
    ...
    for(let i=0;i<left.length;i++){
      const p=left[i],isMeal=restaurant(p),slot=isMeal?meals[mealIndex]:null;
      ...
      walk(left.filter((_,j)=>i!==j),[...route,{...p,mealSlot:slot?.mealSlot||null,startMinute:at,endMinute:at+stay,stayMinutes:stay,incomingTravelMinutes:minutes,travelEstimate:true}],at+stay,length+(prev?km(prev,p):0),mealIndex+(isMeal?1:0));
    }
  }
...
function selectItinerary(pool,{region='hongdae',limit=5,startHour=11,duration=8}={}){
  const pair=CENTERS[region]||CENTERS.hongdae,center={lat:pair[0],lng:pair[1]};
  const ranked=pool.filter(valid).map(p=>({...p,distanceFromRegionKm:km(center,p)})).filter(p=>p.distanceFromRegionKm<=5)
  ...
  for(const seed of ranked.slice(0,20)){
    ...
    const foods=nearby.filter(restaurant).slice(0,mealCount),activities=nearby.filter(p=>!restaurant(p)).slice(0,limit-foods.length);
    ...
    if(!result)continue;
    result.complete=picked.length>=limit&&picked.filter(restaurant).length===mealCount;
    result.missingMeals=mealCount-picked.filter(restaurant).length;
...
module.exports={valid,km,cafe,restaurant,slots,schedule,selectItinerary,CENTERS};
