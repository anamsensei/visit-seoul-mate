(() => {
  ...
  function refreshBadges(){
    document.querySelectorAll('#planner-timeline .tag-cong').forEach((el,i)=>{const p=window.currentDisplayedSpots[i],area=planAreas[p?.id],d=cache.get(area);
      el.className='tag-cong cong-mid';
      el.textContent=area?(fresh(d)?area+' · '+d.crowd.level:area+' · 정보 확인 필요'):'매장 혼잡도 미제공';
    });
  }
  window.cityShouldWarn=plan=>{
    const d=cache.get(planAreas[plan.id]);
    if(!fresh(d))return false;
    const sensitivity=Number(document.querySelector('#mood-selector .active')?.dataset.congestionSens||50);
    return d.crowd.level==='붐빔'||(sensitivity>=80&&d.crowd.level==='약간 붐빔');
  };
