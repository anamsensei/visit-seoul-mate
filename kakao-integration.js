(function () {
  ...
  window.showDistrictKakaoPreview=async function(key){if(!data[key])return;if(state.selected.has(key))state.selected.delete(key);else state.selected.add(key);state.activeKey=null;syncSelection();};
