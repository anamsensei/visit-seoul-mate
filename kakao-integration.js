(function () {
  ...
  function syncSelection(){
    ...
    const card=document.getElementById('district-description-card'),title=document.getElementById('district-description-title'),tag=document.getElementById('district-description-tag'),history=document.getElementById('district-description-history'),feature=document.getElementById('district-description-feature');
    const active=state.activeKey&&data[state.activeKey];
    if(card)card.style.display=active?'block':'none';
    if(active){if(title)title.textContent=`${active.name} · 지역 안내`;if(tag)tag.textContent=active.keyword;if(history)history.textContent=active.history;if(feature)feature.textContent=`추천 포인트: ${active.feature}`;}
